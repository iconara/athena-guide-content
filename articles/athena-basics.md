...
Athena is not an RDBMS or a data warehouse appliance, instead it's a query engine. Unlike most databases Athena doesn't manage the storage of your data, it just provides an interface for querying data stored on S3. While most of your experience with SQL and databases is transferrable to Athena, there are a few things that are distinctly different and that you need to keep in mind to get the most out of it.
...
You can create catalog entries these through DDL statements (i.e. SQL) in Athena, or using the Glue Data Catalog API directly. In theory, tables created in Spark on EMR or Glue ETL will also be available to Athena, although the stars need to align properly for this to actually work flawlessly. When you run DDL statements in Athena it will translate these into Glue Data Catalog API calls behind the scenes.
